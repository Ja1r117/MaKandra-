...
router.get('/users', verifyToken, verifyAdmin, async (req, res) => {
  try {
    const [rows] = await db.query('SELECT id, name, email, role, district, created_at FROM users');
    res.json(rows);
...
router.put('/profile/:id', async (req, res) => {
  try {
    const { first_name, last_name, name: rawName, category, experience, bio, hourly_rate, district, phone, working_hours } = req.body;
    const fn = (first_name || '').trim();
    const ln = (last_name  || '').trim();
    const displayName = fn ? (ln ? fn + ' ' + ln : fn) : (rawName || '').trim();
    if (!displayName) return res.status(400).json({ error: 'Naam is verplicht.' });
    await db.query(
      'UPDATE users SET name=?, first_name=?, last_name=?, category=?, experience=?, bio=?, hourly_rate=?, district=?, phone=?, working_hours=? WHERE id=?',
      [displayName, fn || null, ln || null, category, experience, bio,
       (parseFloat(hourly_rate) || null), district, phone || null, working_hours || null, req.params.id]
    );
    try {
      await db.query(
...
// UPDATE account settings (email, district)
router.put('/account/:id', async (req, res) => {
  try {
    const { current_password, email, district } = req.body;
    if (!email && !district) return res.status(400).json({ error: 'Niets om bij te werken.' });
